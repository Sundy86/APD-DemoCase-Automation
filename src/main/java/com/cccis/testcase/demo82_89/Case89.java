package com.cccis.testcase.demo82_89;
...
public class Case89 extends CommonTest {
    LoginPage loginPage;
    ...
    public void partOperate(String operationType, String partName ) {
        SeleniumUtil.log(log,"----通过配件名称搜索配件" + partName + "，不具备喷漆属性的配件进行喷漆 case89----");

        estimateItemPage.addPartByOperatorByName(operationType,partName);

//        estimateItemPage.selectedPart(partName);
//        estimateItemPage.clickPartOperationTypeBtn(operationType);
        estimateItemPage.partTreeClose();
        estimateItemPage.editItem(1, 12, "1");
        Assert.assertEquals(estimateItemPage.getPromptAlertText(), "不建议对" + partName + "喷漆。");
