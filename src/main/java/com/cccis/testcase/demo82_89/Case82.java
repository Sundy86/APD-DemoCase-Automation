...
public class Case82 extends CommonTest {
    LoginPage loginPage;
    ...
    public void partOperate(String partName ) {
        SeleniumUtil.log(log,"----通过配件名称搜索配件" + partName + "，不具备喷漆属性的配件进行喷漆 case82----");
        SeleniumUtil.log(log,"----选中左后视镜本体");
        estimateItemPage.selectedPart(partName);
        Assert.assertTrue(estimateItemPage.partTreePaintBtnIsdisable(), "无法喷漆");
    }
