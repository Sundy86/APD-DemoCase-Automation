package com.cccis.testcase.demo127_128;
...
public class Case127 extends CommonTest {
    LoginPage loginPage;
    ...
    public void addPart(String flag, String operationType, String partName, String categoryName) {
        SeleniumUtil.log(log,"----通过配件名称搜索配件" + partName + "，非4S店维修不合理添加的辅料 case127----");

        if (flag.equals("1")) {
            SeleniumUtil.log(log,"----点选添加换件 前挡风玻璃切换总成");
            estimateItemPage.addPartByOperatorByCategory(operationType, partName, categoryName);
        }

        if (flag.equals("2")) {
            SeleniumUtil.log(log,"----点选添加 玻璃胶");
