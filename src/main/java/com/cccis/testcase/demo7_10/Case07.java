package com.cccis.testcase.demo7_10;
...
public class Case07 extends CommonTest {
    LoginPage loginPage;
    ...
    public void addPart(String partName, String operationType, String row, String column, String val) {
        SeleniumUtil.log(log,"----通过配件名称搜索配件" + partName + "，配件用量不超标 case07----");

        SeleniumUtil.log(log,"----点选添加换件 后雷达传感器");
        estimateItemPage.addPartByOperatorByName(operationType, partName);
        estimateItemPage.partTreeClose();

        estimateItemPage.editItem(Integer.parseInt(row), Integer.parseInt(column), val);
        Assert.assertEquals(estimateItemPage.getPromptAlertText(), partName + "实际用量超过每总成用量。");
